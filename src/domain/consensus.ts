...
export class Consensus {
    ...
    public async onBid(bid: Bid) {
        Metrics.bidLatency
            .labels(bid.payload.solver)
            .observe((Date.now() - bid.timestamp) / 1000);

        const solver = this.signer.recoverBid(bid);

        if (bid.payload.solver !== solver) {
            this.logger.error(
                `Bid from ${bid.payload.solver} does not match signature, recovered ${solver}`,
            );
            return;
        }
        ...
        this.logger.debug(
            `Received bid: ${JSON.stringify(bid)} from ${solver}`,
        );
    ...
    public async onPrevote(prevote: Prevote) {
        ...
        if (!(await this.validators.getAddresses()).includes(validator)) {
            this.logger.debug(
                `Received prevote from unknown validator: ${validator}`,
            );
            return;
        }

        Metrics.prevoteLatency
            .labels(validator)
            .observe((Date.now() - prevote.timestamp) / 1000);
        this.logger.debug(
        ...
        Metrics.prevoteQuorumLatency.observe(
    ...
    public async onPrecommit(precommit: Precommit) {
        ...
        if (!(await this.validators.getAddresses()).includes(validator)) {
            this.logger.debug(
                `Received precommit from unknown validator: ${validator}`,
            );
            return;
        }

        Metrics.precommitLatency
            .labels(validator)
            .observe((Date.now() - precommit.timestamp) / 1000);
        this.logger.debug(
    ...
    onPrecommitQuorum(
        ...
        Metrics.auctionFinalizationLatency.observe(
            (Date.now() - getBiddingStartTime(auction)) / 1000,
        );
    }
}
